...
import MethodResults from "../../methods/biseccion/methodResults";
...
const biseccion = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (Number.parseFloat(formData.xi) >= Number.parseFloat(formData.xs)) {
        throw new Error("El extremo inferior (xi) debe ser menor que el superior (xs)");
      }

      const requestData = {
        function_text: formData.function,
        xi: Number.parseFloat(formData.xi),
        xs: Number.parseFloat(formData.xs),
        tol: Number.parseFloat(formData.tol),
        max_count: Number.parseInt(formData.max_count),
      };

      api.post("calculations/biseccion/", requestData)
        .then(response => {
      ...
      <header className="bg-gray-900/80 border-b border-gray-800 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2">
          <div className="flex items-center justify-between">
            ...
            <div className="text-white font-medium">Método de Bisección</div>
            <button
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
              ...
              <p className="text-gray-300 mb-4">
                El método de bisección es una técnica numérica para encontrar raíces de una función continua, dividiendo
                repetidamente el intervalo por la mitad y seleccionando el subintervalo donde ocurre el cambio de signo.
              </p>
              <div className="space-y-3 text-gray-400">
                <p>
                  <span className="font-medium text-teal-400">Funcionamiento:</span> Divide el intervalo y selecciona
                  el subintervalo que contiene la raíz.
                </p>
                <p>
                  <span className="font-medium text-teal-400">Ventajas:</span> Método robusto y confiable.
                </p>
                <p>
                  <span className="font-medium text-teal-400">Requisitos:</span> La función debe ser continua y cambiar
                  de signo en el intervalo [xi, xs].
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
              <MethodFormTemplate
                ...
                onAChange={(e) => handleChange({ target: { name: "xi", value: e.target.value } })}
                onBChange={(e) => handleChange({ target: { name: "xs", value: e.target.value } })}
