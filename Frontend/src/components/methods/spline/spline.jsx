...
import MethodFormSpline from "./methodForm";
import MethodResultsSpline from "./methodResults";
import api from "../../../api/config";

const Spline = () => {
  const [formData, setFormData] = useState({
    xPoints: "-2,-1,2,3",
    yPoints: "12.13533528,6.367879441,-4.610943901,2.085536923",
    degree: 3,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const x = formData.xPoints.split(",").map(Number);
      const y = formData.yPoints.split(",").map(Number);
      const degree = Number(formData.degree);
      ...
      api.post("calculations/spline/", requestData)
        .then(response => {
      ...
      <header className="bg-gray-900/80 border-b border-gray-800 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2">
          <div className="flex items-center justify-between">
            ...
            <div className="text-white font-medium">Método Spline</div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
              ...
              <p className="text-gray-300 mb-4">
                Calcula los polinomios de interpolación por tramos utilizando splines lineales o cúbicos.
              </p>
              <div className="space-y-3 text-gray-400">
                <p>
                  <span className="font-medium text-teal-400">Entrada:</span> Conjunto de puntos (x, y) y el grado (1 o 3).
                </p>
                <p>
                  <span className="font-medium text-teal-400">Salida:</span> Polinomios por tramo y gráfica correspondiente.
                </p>
                <p>
                  <span className="font-medium text-teal-400">Recomendación:</span> Se sugiere ingresar puntos ordenados por X para resultados más claros.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
              <MethodFormSpline
                ...
                onDegreeChange={(e) => handleChange({ target: { name: "degree", value: e.target.value } })}
            ...
            <div>
              {results ? (
                <MethodResultsSpline results={results} />
              ) : (
                <div className="bg-gray-800/50 rounded-xl border border-gray-700 p-8 flex flex-col items-center justify-center h-full">
                  ...
                  <p className="text-gray-400 text-center">Ingrese los datos y haga clic en "Calcular" para ver el resultado.</p>
                </div>
              )}
            </div>
          </div>
